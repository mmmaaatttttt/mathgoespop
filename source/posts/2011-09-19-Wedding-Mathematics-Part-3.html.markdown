...
[caption id="attachment_1392" align="aligncenter" width="604" caption="Table number e. Hat tip to Caroline for the shot."]<a href="http://www.mathgoespop.com/images/2011/09/307071_10150266178495592_518300591_7799467_6534693_n.jpg"><img class="size-full wp-image-1392" title="etable" src="http://www.mathgoespop.com/images/2011/09/307071_10150266178495592_518300591_7799467_6534693_n.jpg" alt="" width="604" height="452" /></a>[/caption]
...
1. Show that $latex \zeta(1)=\infty$.

2. Given that $latex \zeta(2)=\frac{\pi^{2}}{6}$, show that $latex 1+\frac{1}{3^{2}}+\frac{1}{5^{2}}+\frac{1}{7^{2}}+\ldots=\frac{\pi^{2}}{8}$.

<strong>4. $latex \gamma$ (see <a href="http://en.wikipedia.org/wiki/Euler%E2%80%93Mascheroni_constant">here</a> for more).</strong>

$latex \gamma$, a.k.a. the Euler-Mascheroni constant (not to be confused with Euler's number <em>e</em>), is perhaps best introduced geometrically. Consider the following figure:

<a href="http://www.mathgoespop.com/images/2011/09/Picture-1.png"><img class="aligncenter size-full wp-image-1402" title="gammapic" src="http://www.mathgoespop.com/images/2011/09/Picture-1.png" alt="" width="600" height="398" /></a>The black portion of the area pictured above is found by drawing rectangles between two integers <em>n</em> and <em>n</em> + 1 with height 1/<em>n</em> (the rectangle between 1 and 2 has height 1, the rectangle between 2 and 3 has height 1/2, and so on), and subtracting the area under the graph of the function <em>y</em> = 1/<em>x</em>.  The total black area, if this picture were to be extented out to infinity, would represent the number $latex \gamma$.

$latex \gamma$ can be approximated by its decimal expansion, $latex \gamma\approx0.5772\ldots$, and while this number comes up quite naturally in number theory and mathematical statistics, surprisingly little is known about it. For example, it is unknown whether or not $latex \gamma$ is a rational number (unlike constants such as $latex \pi$ or <em>e</em>, which are known to be irrational).

Table exercises!

1. Using geometry and the figure above, show that $latex \gamma&gt;\frac{1}{4}+\frac{1}{12}+\frac{1}{24}+\frac{1}{40}+\ldots$.

2. Show that the sum on the right hand side of the inequality in the first exercise equals $latex \frac{1}{2}$, so that $latex \gamma&gt;\frac{1}{2}$.

<strong>5. $latex \infty$ (see <a href="http://en.wikipedia.org/wiki/Infinity">here</a> for more).</strong>

$latex \infty$ is a concept of central importance in mathematics, and ergo, a concept of central importance in all things. While the figure-eight symbol for infinity is known and loved by all, it was not introduced until the year 1655, though many ancient cultures grappled with the idea of the infinite.

Though $latex \infty$ may seem like a single idea, great minds have shown that not all infinities are created equal. For example, the mathematician Georg Cantor showed that even though there are infinitely many whole numbers, and there are infinitely many real numbers, there are (in a sense that can be made rigorous) infinitely many more real numbers than counting numbers.
...
3. Suppose a set is finite with <em>N</em> elements. Show that the set of subsets of the original set is finite with $latex 2^{N}$ elements.

<strong>6. $latex \varphi$ (see <a href="http://en.wikipedia.org/wiki/Golden_ratio">here</a> for more).</strong>

Suppose two line segments have length <em>a</em> and <em>b</em>, with <em>a</em> larger than <em>b</em>. If the ratio of <em>a</em> to <em>b</em> is the same as the ratio of <em>a</em> + <em>b</em> to <em>b</em>, this ratio is called the golden ratio, and is written $latex \varphi$. In other words,
<p style="text-align: center;">$latex \varphi=\frac{a}{b} = \frac{a+b}{a} = 1 + \frac{1}{\varphi}$.</p>
This, in turn, implies that $latex \varphi^{2}-\varphi-1=0$, or (by the quadratic formula)
<p style="text-align: center;">$latex \varphi=\frac{1+\sqrt{5}}{2}\approx1.618\ldots$.</p>
<p style="text-align: left;">The golden ratio has a rich history, both mathematically and artistically. It is also closely related to the Fibonacci sequence, the sequence of numbers whose first two terms are 0 and 1, and where all subsequent terms are found by adding the previous two terms. In other words, the sequence begins $latex 0,1,1,2,3,5,8,13,\ldots$. If we let $latex F_{n}$ denote the $latex n^{th}$ Fibonacci number (so $latex F_{0}=0, F_{7}=13$, and so on), then</p>
<p style="text-align: center;">$latex \varphi=\lim_{n\rightarrow\infty}\frac{F_{n+1}}{F_{n}}.$</p>
...
<p style="text-align: center;">$latex \rho=\sqrt[3]{\frac{1}{2}+\frac{1}{6}\sqrt{\frac{23}{3}}}+\sqrt[3]{\frac{1}{2}-\frac{1}{6}\sqrt{\frac{23}{3}}}\approx1.3247\ldots.$</p>
